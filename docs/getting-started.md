...
This page walks you through setting up Callisto in your server for the first time. You can also run `c!setup` at any time to check your setup status.
...
```bash
c!config set staffrole @StaffRole
```
...
```bash
c!config set logchannel #channel
```
...
```bash
c!category add support Support #channel
```
...
Once created, run `c!category list` to confirm it was set up correctly.
...
Members can now open tickets with `c!ticket open <category>` or by reacting to the panel if you set one up. Run `c!setup` at any time to check your configuration status.